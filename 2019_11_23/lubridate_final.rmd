...
#А какие еще есть?
head(OlsonNames(), 10)
#указать, в каком часовом поясе мы находимся, без изменения времени
force_tz(date, tzone = "Europe/Moscow")

#уточнить, то же время но в другом часовом поясе
...
```{r}
data = read.csv2("/Users/aleksandra/Desktop/2018-2019/ЕУ/митап/митап_2019/dplyr/shengen/data_2017.csv")
```
...
data %>% 
 mutate(month = month(crash_date, label = TRUE)) %>% 
  ...
  filter(crash_type_name == "Столкновение" | crash_type_name == "Опрокидывание" | crash_type_name == "Наезд на пешехода") %>% 
...
data %>% 
 mutate(month = month(crash_date, label = TRUE)) %>% 
  ...
  xlab("")+
   labs(
    title = "Что случилось с июлем?",
    subtitle = paste0('Я не ожидала такой подставы от июля.\n', "Вообще, строго говоря, вопрос нетривиальный.\n", "Я бы поняла отсутствие данных с обоих концов года, но июль..."))
...
* [Как гуглить русский R](http://mtweb.cs.ucl.ac.uk/mus/bin/install_R/R-3.1.1/src/library/base/po/ru.po)
